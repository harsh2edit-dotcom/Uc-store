...
// ============ HELPERS ============
const isAdmin = (id) => String(id) === String(ADMIN_CHAT_ID);

function ensureUser(id, name) {
  const users = readJSON(USERS_FILE, {});
  if (!users[id]) {
    users[id] = { username: name || 'User', balance: 0, createdAt: new Date().toISOString() };
...
bot.on('callback_query', async (q) => {
  ...
  if (data === 'buy_uc') {
    ...
    bot.editMessageText('✦ ◈ CHOOSE YOUR PLAN ◈ ✦\n\n⟡ Tap a plan to continue ⟡', { chat_id: chatId, message_id: msgId, ...kb });
    return;
  }
  ...
  if (data.startsWith('plan_')) {
    ...
    const s = getSession(chatId);
    await showLoading(chatId, msgId, 'Checking balance');
  ...
  if (data === 'coupon') {
    ...
    bot.editMessageText(
      ...
      '⚠ <i>Coupon use karne ke liye minimum deposit zaroori hai</i>',
      {
        chat_id: chatId, message_id: msgId, parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🟠 ⟣ BACK ⟢', callback_data: 'main_menu' }]] }
      }
  ...
  if (data === 'my_orders') {
    await showLoading(chatId, msgId, 'Fetching activity');
  ...
  if (data.startsWith('adm_')) {
    ...
    if (action === 'approve_order') {
      ...
      if (o) bot.sendMessage(o.chatId, '✦ 🟢 ORDER APPROVED ✦\n\n◈ Order: #' + id + '\n◈ Plan: ' + o.plan.uc + ' UC\n\n⟡ UC will be delivered shortly ⟡');
      return;
    }
    ...
    if (action === 'reject_order') {
      ...
      bot.editMessageText('🔴 ❌ Order ' + id + ' REJECTED + ₹' + (o ? o.plan.price : 0) + ' refunded', { chat_id: chatId, message_id: msgId });
      if (o) bot.sendMessage(o.chatId, '✦ 🔴 ORDER REJECTED ✦\n\n◈ Order: #' + id + '\n◈ Refund: ₹' + o.plan.price + '\n\n⟡ Amount credited back ⟡');
      return;
    }
    ...
    if (action === 'approve_dep') {
      ...
      bot.editMessageText('🟢 ✅ Deposit ' + id + ' APPROVED + ₹' + (d ? d.amount : 0) + ' credited', { chat_id: chatId, message_id: msgId });
      if (d) bot.sendMessage(d.chatId, '✦ 🟢 DEPOSIT APPROVED ✦\n\n◈ Amount: ₹' + d.amount + '\n\n⟡ Balance credited ⟡');
      return;
    }
    ...
    if (action === 'reject_dep') {
      ...
      if (d) bot.sendMessage(d.chatId, '✦ 🔴 DEPOSIT REJECTED ✦\n\n◈ Amount: ₹' + d.amount + '\n\n⟡ Contact support ⟡');
      return;
    }
  }
});
...
bot.on('message', async (msg) => {
  ...
  // ADMIN MODE
  if (s.state === 'admin') {
    if (text === '/exit') { clearSession(chatId); bot.sendMessage(chatId, '🟠 ⟣ Exited admin mode'); return; }
  ...
  if (s.state === 'awaiting_wp') {
    ...
    const loadingMsg = await bot.sendMessage(chatId, '✦ ⏳ PLACING ORDER... ✦\n\n▓▓▓▓▓▓▓▓░░░░░░░░');
    await sleep(800);
    ...
    bot.sendMessage(ADMIN_CHAT_ID,
      ...
      '◈ WP: <code>' + s.data.wp + '</code>',
      {
        ...
        reply_markup: {
          inline_keyboard: [[
            { text: '🟢 ✅ APPROVE', callback_data: 'adm_approve_order_' + orderId },
  ...
  if (s.state === 'awaiting_utr') {
    ...
    const loadingMsg = await bot.sendMessage(chatId, '✦ ⏳ SUBMITTING... ✦\n\n▓▓▓▓▓▓▓▓░░░░░░░░');
    await sleep(800);
  ...
  if (s.state === 'awaiting_coupon') {
    ...
    const coupons = readJSON(COUPON_FILE, []);
    ...
    if ((coupon.used || 0) >= coupon.maxUses) {
    ...
    const deps = readJSON(DEPOSITS_FILE, []);
    const userDepositTotal = deps
      .filter(d => String(d.chatId) === String(chatId) && d.status === 'approved')
      .reduce((s, d) => s + d.amount, 0);

    if (userDepositTotal < (coupon.minDeposit || 200)) {
      bot.sendMessage(chatId,
        '✦ ⚠ COUPON LOCKED ✦\n\n' +
        '🟡 This coupon requires minimum deposit of <b>₹' + (coupon.minDeposit || 200) + '</b>\n\n' +
        '🟢 Your total deposit: <b>₹' + userDepositTotal + '</b>\n\n' +
        '⟡ Please deposit first to use this coupon ⟡',
        {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [[
    ...
    writeJSON(COUPON_FILE, coupons);

    const value = coupon.value;
    const newBal = updateBalance(chatId, value);
    ...
    bot.sendMessage(ADMIN_CHAT_ID,
      ...
      '◈ Code: <b>' + code + '</b>\n' +
      '◈ Value: ₹' + value + '\n' +
      '◈ Remaining Uses: ' + (coupon.maxUses - coupon.used),
      { parse_mode: 'HTML' }
    );
...
app.listen(process.env.PORT || 3000, () => console.log('✅ Keep-alive started on port ' + (process.env.PORT || 3000)));
